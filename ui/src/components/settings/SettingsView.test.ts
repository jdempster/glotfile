...
vi.mock("@/api.js", () => ({
  ...
  suggestProjectContext: vi.fn(() => Promise.resolve({ projectContext: "Acme is a visitor-management system." })),
...
describe("SettingsView translation guidance", () => {
  ...
  it("saves the project context and per-locale instruction into the config", async () => {
    ...
    await w.get("#project-context").setValue("Acme is a workplace visitor-management system.");
    ...
    expect(putConfig).toHaveBeenCalledWith(expect.objectContaining({
      projectContext: "Acme is a workplace visitor-management system.",
  ...
  it("fills the project-context field from an AI suggestion", async () => {
    ...
    expect((w.get("#project-context").element as HTMLTextAreaElement).value).toBe("Acme is a visitor-management system.");
    w.unmount();
  });
  ...
  it("fills a locale field from an AI suggestion, passing the current project context", async () => {
    ...
    await w.get("#project-context").setValue("Acme is a visitor system.");
    await w.get("[data-testid='suggest-locale-fr']").trigger("click");
    await flushPromises();

    expect(suggestLocaleInstruction).toHaveBeenCalledWith({ locale: "fr", projectContext: "Acme is a visitor system." });
