...
export const CHECK_IDS = ["untranslated", "placeholder", "spelling", "length", "glossary"] as const;
...
// everywhere: a rule turned "off" in config.lint.rules, or suppressed per
// key+locale, silences the matching live editor issue too.
...
export function runChecks(state: State, opts: CheckOptions = {}): CheckResult {
  const ruleOff = (id: CheckId) => state.config.lint?.rules?.[CHECK_RULE[id]] === "off";
  const on = (id: CheckId) => (!opts.only || opts.only.includes(id)) && !ruleOff(id);
  const issues: Issue[] = [];
