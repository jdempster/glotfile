...
describe("extractLiterals", () => {
  it("finds a key-shaped literal in a ternary assignment", () => {
    const content = "$key = $cond\n  ? 'sms/visitor-arrival.delivery_message'\n  : 'sms/visitor-arrival.message';";
    const lits = extractLiterals(content);
    expect(lits).toEqual([
      expect.objectContaining({ literal: "sms/visitor-arrival.delivery_message", line: 2 }),
      expect.objectContaining({ literal: "sms/visitor-arrival.message", line: 3 }),
    ]);
  });
