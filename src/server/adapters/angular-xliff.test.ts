...
describe("angular-xliff", () => {
  ...
  it("renders an ICU-apostrophe literal as plain text, only the real placeholder as an x-element", () => {
    const s = defaultState();
    s.config.locales = ["en", "fr"];
    createKey(s, "literal", "Dear {visitor}, see '{site}'");
    setTargetValue(s, "literal", "fr", "Cher {visitor}, voir '{site}'");
    const fr = contents(angularXliff.export(s, OUT), "fr");
    // The real {visitor} becomes one <x/>; the literal '{site}' is escaped text,
    // never an interpolation placeholder.
    expect(fr).toContain(
      '<source>Dear <x id="INTERPOLATION" equiv-text="{{visitor}}"/>, see \'{site}\'</source>',
    );
    expect(fr).toContain(
      '<target>Cher <x id="INTERPOLATION" equiv-text="{{visitor}}"/>, voir \'{site}\'</target>',
    );
  ...
  it("round-trips an ICU-apostrophe literal (import(export(x)) === x)", () => {
    const s = defaultState();
    s.config.locales = ["en", "fr"];
    const en = "Dear {visitor}, see '{site}'";
    const frVal = "Cher {visitor}, voir '{site}'";
