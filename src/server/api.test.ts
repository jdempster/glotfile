...
describe("POST /guidance/suggest", () => {
  function setupGuidance(makeProvider?: () => unknown) {
    ...
    createKey(s, "visitor.evacuate", "Evacuate");
    saveState(file, s);
  ...
  const provider = () => ({
    translate: async () => [],
    supportsVision: () => false,
    complete: async () => ({ projectContext: "Acme is a workplace visitor-management system.", instruction: "Use vouvoiement." }),
  });
  ...
  it("suggests a project context from the catalog", async () => {
    ...
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ projectContext: "Acme is a workplace visitor-management system." });
  });
  ...
  it("suggests a per-locale instruction grounded in the supplied project context", async () => {
    ...
    const res = await app.request("/guidance/suggest/locale", {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ locale: "fr", projectContext: "Acme is a visitor system." }),
    });
    expect(res.status).toBe(200);
