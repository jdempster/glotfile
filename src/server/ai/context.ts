...
export function buildContextSystemPrompt(guidance: ContextGuidance = {}): string {
  ...
  const lines = [
    ...
    "- Domain terms: when the source uses a product/domain term (especially one the project context defines), keep the term VERBATIM in your note — never paraphrase it into a synonym or an English gloss (e.g. don't rewrite 'feed' as 'give fertilizer'). The translation engine applies per-language wording rules keyed to the exact term, and a paraphrase in the note steers translations away from them. Name the intended SENSE while keeping the term itself.",
