...
export function buildChatSystemPrompt(): string {
  return [
    ...
    "",
    "You have tools to read the project state, search and read the user's CODEBASE (README, components, locale files — read-only), and to make FOCUSED changes: the project context and per-language rules; glossary terms (and accepting or dismissing glossary suggestions); a key's context note or notes; and fixing or re-marking one individual translation. Prefer to LOOK before you write:",
    ...
    "- The snapshot below already surfaces the main gaps (missing context, languages without rules, glossary size, pending suggestions, translation progress) — use it to decide what to tackle first without spending a tool call; only read deeper (the codebase, specific keys) when you need detail to draft a proposal.",
