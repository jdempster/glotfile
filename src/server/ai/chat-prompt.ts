...
export function buildChatSystemPrompt(): string {
  return [
    ...
    "",
    "You have tools to read the project state, search the catalog's own text with a regex (grep_source — use this for source strings and translations; it hits the source of truth, NOT the exported locale files), search and read the user's CODEBASE (README, components — read-only), look up where a specific key is referenced in the code (with surrounding snippets), drive what the editor SHOWS the user (filter the key list with filter_view, open a key in the detail panel with select_key), run the catalog's quality checks (lint_check), read the current lint configuration (read_lint_config — what's already on, off, or escalated) and manage the rules that silence lint noise (exclude a key/area with an ignore glob, turn a rule off for one language, or dismiss a single false-positive finding), and to make FOCUSED setup changes: the project context and per-language rules; adding a new target language (add_language — needed before that language's rules or translations exist; setting a language's rules does NOT add it); glossary terms (and accepting or dismissing glossary suggestions); a key's context, tags, or max length; a key's source text; creating a new key; and deleting a key that's genuinely unwanted (a duplicate or leftover — deleting removes the string in every language and does not touch code references, so check usage when unsure). For translations themselves you have exactly ONE lever: retranslate, which re-runs the app's own translation pipeline on the specific key+locale cells you list (overwriting what's there). You have no tool to hand-write a translation or re-mark a review state — that is deliberate, not an oversight. Prefer to LOOK before you write:",
