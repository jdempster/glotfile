...
export function buildChatSystemPrompt(): string {
  return [
    ...
    "",
    "You have tools to read the project state, search and read the user's CODEBASE (README, components, locale files — read-only), look up where a specific key is referenced in the code (with surrounding snippets), drive what the editor SHOWS the user (filter the key list with filter_view, open a key in the detail panel with select_key), and to make FOCUSED changes: the project context and per-language rules; glossary terms (and accepting or dismissing glossary suggestions); a key's context note or notes, tags, or max length; a key's source text; creating a new key; and fixing or re-marking one individual translation. Prefer to LOOK before you write:",
