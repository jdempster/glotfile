...
describe("computeUsedKeys", () => {
  ...
  it("treats an exact literal match as used", () => {
    const s = stateWith("sms/visitor-arrival.message", "dead.key");
    expect(computeUsedKeys(s, cacheWithLiterals("sms/visitor-arrival.message"))).toEqual(["sms/visitor-arrival.message"]);
  });

  it("treats a literal as a prefix for keys nested under it", () => {
    const s = stateWith("emails/visitor-arrival.delivery.title", "emails/visitor-arrival.standard.title", "dead.key");
    expect(computeUsedKeys(s, cacheWithLiterals("emails/visitor-arrival.delivery"))).toEqual(["emails/visitor-arrival.delivery.title"]);
  });
